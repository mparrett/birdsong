...
def generate_symbol_audio_8(symbol, config):
    ...
    if symbol == 0:  # 000: Low Rising
        return generate_frequency_sweep(
            config.low_freq, config.mid_freq, duration, config.sample_rate
        )
    elif symbol == 1:  # 001: Low Falling
        return generate_frequency_sweep(
            config.mid_freq, config.low_freq, duration, config.sample_rate
        )
    elif symbol == 2:  # 010: Mid Rising
        return generate_frequency_sweep(
            config.mid_freq, config.high_freq, duration, config.sample_rate
        )
    elif symbol == 3:  # 011: Mid Falling
        return generate_frequency_sweep(
            config.high_freq, config.mid_freq, duration, config.sample_rate
        )
    elif symbol == 4:  # 100: High Rising
        return generate_frequency_sweep(
            config.high_freq, config.top_freq, duration, config.sample_rate
        )
    elif symbol == 5:  # 101: High Falling
        return generate_frequency_sweep(
            config.top_freq, config.high_freq, duration, config.sample_rate
        )
    elif symbol == 6:  # 110: Cross Rising (wide sweep)
        return generate_frequency_sweep(
            config.low_freq, config.high_freq, duration, config.sample_rate
        )
    elif symbol == 7:  # 111: Cross Falling (wide sweep)
        return generate_frequency_sweep(
            config.high_freq, config.low_freq, duration, config.sample_rate
        )
    else:
...
def detect_symbol_8(audio_segment, config):
    ...
    f, t, Sxx = signal.spectrogram(
        audio_segment, config.sample_rate, nperseg=nperseg, noverlap=noverlap
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="8-Symbol Frequency Sweep Transmission"
    )
    ...
    print(
        f"  Frequency bands: {config.low_freq:.0f}-{config.mid_freq:.0f}-{config.high_freq:.0f}-{config.top_freq:.0f}Hz"
    )
