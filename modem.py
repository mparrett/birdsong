...
class AcousticModem:
    ...
    def find_dominant_bit(self, data: np.ndarray) -> Optional[int]:
        ...
        if (
            mag_start > self.amplitude_threshold
            and mag_start > mag1
            and mag_start > mag0
        ):
            return 2  # Handshake
    ...
    def recv(self, input_file: str):
        ...
        try:
            ...
            with wave.open(input_file, "rb") as wav_file:
                ...
                if sample_rate != self.sample_rate:
                    print(
                        f"Error: Expected sample rate {self.sample_rate}, got {sample_rate}",
                        file=sys.stderr,
                    )
                    sys.exit(1)
        ...
        if received_checksum != expected_checksum:
            print(
                f"Error: Checksum mismatch! Expected {expected_checksum}, got {received_checksum}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def main():
    ...
    parser_send.add_argument(
        "-s", "--speed", type=float, default=1.0, help="Speed multiplier"
    )
    ...
    if args.speed <= 0:
        print(
            f"Error: Speed multiplier must be positive, got {args.speed}",
            file=sys.stderr,
        )
        sys.exit(1)
