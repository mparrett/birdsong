...
Baseline version: 2 bits per symbol, ~25 symbols/sec = 50 bits/s
...
def generate_frequency_sweep(start_freq, end_freq, duration, sample_rate, phase_offset=0):
    """
...
def generate_symbol_audio(symbol, config, phase_offset=0):
    ...
    if symbol == 0:  # 00: Low Rising
        return generate_frequency_sweep(config.low_freq_start, config.low_freq_end, 
                                      duration, config.sample_rate, phase_offset)
    elif symbol == 1:  # 01: Low Falling
        return generate_frequency_sweep(config.low_freq_end, config.low_freq_start,
                                      duration, config.sample_rate, phase_offset)
    elif symbol == 2:  # 10: High Rising
        return generate_frequency_sweep(config.high_freq_start, config.high_freq_end,
                                      duration, config.sample_rate, phase_offset)
    elif symbol == 3:  # 11: High Falling  
        return generate_frequency_sweep(config.high_freq_end, config.high_freq_start,
                                      duration, config.sample_rate, phase_offset)
    else:
    ...
    for i, symbol in enumerate(symbols):
        ...
        if i == 0:
            ...
            if main_start < total_samples:
                ...
                audio[main_start:main_start + main_samples] = symbol_audio[overlap_samples:overlap_samples + main_samples]
...
def main():
    ...
    parser.add_argument('mode', choices=['send', 'recv'], 
                       help='Send or receive sweep data')
    
    # Send mode arguments
    parser.add_argument('--text', '-t', default="Hi",
                       help='Text to send (default: Hi)')
    ...
    # Configuration
    parser.add_argument('--symbol-duration', type=float, default=0.040,
