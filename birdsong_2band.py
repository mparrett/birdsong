...
def command_send(output_file, bit_duration, freq_config):
    ...
    if freq_config.multi_band:
        ...
        log.info(f"Sender: Multi-band CPFSK transmitting {len(symbols_to_transmit)} symbols (2 bits each).")
        log.info(f"Sender: Data rate: {len(payload_bits + checksum_bits) / (len(symbols_to_transmit) * bit_duration):.1f} bits/s")
