    ...
    for i, freq in enumerate(our_frequencies):
        ...
        for peak_idx in peaks[:5]:  # Top 5 peaks
            ...
            if peak_freq > 10:  # Ignore DC
                print(
                    f"    {peak_freq:.1f}Hz: {20 * np.log10(peak_mag / np.max(mags)):.1f}dB"
                )
...
def analyze_bitmap_audio(wav_file):
    ...
    for expected in our_frequencies:
        ...
        print(
            f"  Expected {expected:.1f}Hz → Found {closest_freq:.1f}Hz ({10 * np.log10(closest_power):.1f}dB)"
        )
...
def main():
    ...
    parser.add_argument(
        "command",
        choices=["harmonics", "crosstalk", "analyze", "natural"],
